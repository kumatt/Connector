...
EventBus 采用发布-订阅模式，主要包含以下核心组件：

1. `AnyEvent` 协议：定义事件的基本行为
   - `send`: 发送事件
   - `sink`: 订阅事件
...
```swift
let cancellable = EventCenter.UserEvent.sink(context: .default) { event in
    switch event {
...
```swift
EventCenter.UserEvent.send(context: .default, .login(userId: "12345")) 
```
